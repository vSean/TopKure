...
'<html>
	...
	<body>
		<span class="pagetitle">Installing Top Kure ' . $version . '</span><br/>
		<p>Detecting tasks...</p>
';
...
if(!is__writable('./'))
	$todo_user[] = 'Please make the current directory (<tt>' . dirname($_SERVER['SCRIPT_NAME']) . '</tt>) writeable';

if(!file_exists('config.php'))
	$todo_kure['config'] = 'Create file: <tt>config.php</tt>';

if(!file_exists('entries/'))
	$todo_kure['entries'] = 'Create directory: <tt>entries</tt>';
elseif(!is__writable('entries'))
	$todo_user[] = 'Change Permissions: <tt>entries/</tt> (writeable)';
...
if(isset($todo_user)) {
	...
	print 'Everything looks fine... Top Kure is ready to do the following...';
...
if(!isset($todo_user)) {
    ...
    print '<span class="pagetitle" style="color: #000000; font-size: 16px;">Please Enter Password:</span>';
		print '<p><input type="password" name="pass1"> (New Password)</p><p><input type="password" name="pass2"> (Confirm New Password)</p>';
